...
object Permission {
     ...
     *
     * 跳转拨号页面:ACTION_DIAL   直接拨打电话:ACTION_CALL
     * mContext.startActivity(Intent(Intent.ACTION_CALL,Uri.parse("tel:$stuPhone")))
     */
    fun Context.getCallPhone(action: (permissions: Array<String>) -> Unit) {
        val tip = TipInfo("注意:", getString(R.string.rationale_call_phone), "不让看", "打开权限")
        PermissionsUtil.requestPermission(
            this, object : PermissionListener {
                override fun permissionGranted(@NonNull permissions: Array<String>) {
                    LogUtils.e(this::class.java, "TODO: CALL_PHONE Granted")
                    action.invoke(permissions)
                }

                override fun permissionDenied(@NonNull permissions: Array<String>) {
                    LogUtils.e(this::class.java, "TODO: CALL_PHONE Denied")
                }
    ...
    inline fun <reified T : Activity> Activity.getScanCamere(
        mContext: Context, tipInfo: TipInfo? = TipInfo(
            "注意:", mContext.getString(R.string.rationale_call_camere), "不让看", "打开权限"
        )
    ) {
        PermissionsUtil.requestPermission(
            mContext, object : PermissionListener {
                override fun permissionGranted(@NonNull permissions: Array<String>) {
    ...
    fun Activity.toGetRead_Write(
        filePath: String, fileName: String, tipInfo: TipInfo? = TipInfo(
            "注意:", this.getString(R.string.rationale_file), "不让看", "打开权限"
        )
    ) {
        PermissionsUtil.requestPermission(
            this, object : PermissionListener {
                override fun permissionGranted(@NonNull permissions: Array<String>) {
                    FileDisplayActivity.actionStart(
                        this@toGetRead_Write, filePath, fileName
                    )
                }
